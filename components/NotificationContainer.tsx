...
const TONE: Record<string, { border: string; text: string }> = {
  ...
  info: { border: 'border-slate-700', text: 'text-slate-200' },
};
...
const NotificationContainer: React.FC<{ notifications: NotificationType[]; onRemove: (id: number) => void }> = ({ notifications, onRemove }) => {
  ...
  return (
    <View pointerEvents="box-none" style={{ position: 'absolute', top: 8, left: 12, right: 12, zIndex: 50 }}>
        ...
        return (
          <FadeInView key={n.id}>
            <Pressable
              ...
              className={`bg-slate-900/95 border rounded-2xl px-4 py-3 mb-2 ${tone.border}`}
            >
